...
public class CommandCreator {
	...
	static final Logger myLogger = LoggerFactory.getLogger(CommandCreator.class
			.getName());
	private Class<? extends GricliCommand> command;

	private final HashMap<String, CommandCreator> keywords = new HashMap<String, CommandCreator>();
	...
	public CommandCreator addConstructor(
	...
	public GricliCommand create(String[] tokens) throws SyntaxException {
		final PartialCommand pc = new PartialCommand();
		CommandCreator c = this;
		for (final String token : tokens) {
			c = c.nextToken(pc, token);
		}
		if ((c.cons == null) && (c.argCreator != null)) {
			final String command = StringUtils.join(pc.keywords.toArray(), " ");
			throw new NotEnoughArgumentsException("not enough arguments for \""
					+ command + "\"");
		} else if (c.cons == null) {
	...
	private CommandCreator nextToken(PartialCommand pc, String nextToken)
		...
		if (((pc.arguments.size() > 0) || (this.cons != null))
				&& (command.length() > 0)) {
			throw new TooManyArgumentsException("too many arguments for \""
					+ command + "\"");
		} else {
