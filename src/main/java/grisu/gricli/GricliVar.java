...
public enum GricliVar {
	...
	WALLTIME("walltime"),VERSION("version"),DEBUG("debug") ,JOBNAME("jobname"), CPUS("cpus"), JOBTYPE("jobtype") {
		...
		public void setValue(String value) throws GricliSetValueException {
			if (!"mpi".equals(value) && !"single".equals(value)
					&& !"threaded".equals(value)) {
				throw new GricliSetValueException("jobtype", value,
						"must be one of 'mpi', 'single' or 'threaded'");
			}
			super.setValue(value);
		}
	},
	MEMORY("memory") {
		...
		public void setValue(String value) throws GricliSetValueException {
			try {
				...
				if (ivalue < 0) {
					throw new GricliSetValueException("memory", value,
							"cannot be negative");
				}
				super.setValue(value);
	...
	FQAN("group"), HOST("host"), GRID_DIR("gdir"), LOCAL_DIR("dir") {
		...
		public void setValue(String value) throws GricliSetValueException {
			try {
				...
				if (!dir.exists()) {
					throw new GricliSetValueException("dir",
	...
	EMAIL("email"), PROMPT("prompt"), ATTACHED_FILES("files") {
	...
	private static final Map<String, GricliVar> lookup = new HashMap<String, GricliVar>();
	...
	public void add(String value) throws GricliSetValueException {
		throw new GricliSetValueException(this.var, "[]",
				"cannot add value to variable that is not a list");
	}
