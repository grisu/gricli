...
public class GrisuClientCommandlineProperties implements GrisuClientProperties {
	...
	private static Options getOptions() {
		...
		// common options
		final Option config = createOptionWithArg(CONFIG_FILE_PATH_OPTION, "F",
				"configuration file location (optional, default: "
						+ GrisuClientFileConfiguration.CONFIG_FILE_PATH + ")");
		final Option serviceInterfaceUrl = createOptionWithArg(
		...
		final Option mode = createOptionWithArg(
				MODE_OPTION,
				"m",
				"the mode you want to use: all|submit|check|join|force-clean|login (optional, default: all)");
		final Option baseName = createOptionWithArg(
				CommandlineProperties.JOBNAME, "n",
				"the name for the job (required)");
		final Option myproxy_username = createOptionWithArg(
				MYPROXY_USERNAME_OPTION,
				"myproxy username (required unless local proxy is used)");
		final Option useLocalProxy = createOption(USE_LOCAL_PROXY_OPTION, "l");
		final Option email = createOptionWithArg(
				CommandlineProperties.EMAIL_OPTION,
				"the email address to send status report to");
		final Option verbose = createOption(VERBOSE_OPTION, "v",
				"verbose output");
		final Option debug = createOption(DEBUG_OPTION,
				"more debug output to $HOME/.grisu/grisu.debug (optional)");
		final Option submissionLocation = createOptionWithArg(
		...
		final Option vo = createOptionWithArg(CommandlineProperties.VO_OPTION,
				"the vo to use (required for modes all|submit)");
		final Option command = createOptionWithArg(
				CommandlineProperties.COMMAND_OPTION, "c",
				"the commandline to run remotely (required for modes all|submit)");
		final Option baseInputFilePath = createOptionWithArg(
				CommandlineProperties.INPUTFILEPATH_OPTION,
				"the input files, separated with a coma");
		final Option walltime = createOptionWithArg(
				CommandlineProperties.WALLTIME_OPTION,
				"the walltime in seconds (required for modes all|submit)");
		final Option cpus = createOptionWithArg(
				CommandlineProperties.CPUS_OPTION,
				"the number of cpus to run the job with (required for modes all|submit)");
		final Option memory = createOptionWithArg(
				CommandlineProperties.MEMORY_OPTION, "r",
				"the number of cpus to run the job with (required for modes all|submit)");
		final Option stdout = createOptionWithArg(
				CommandlineProperties.STDOUT_OPTION,
				"the name of the stdout file (optional)");
		final Option stderr = createOptionWithArg(
				CommandlineProperties.STDERR_OPTION,
				"the name of the stderr file (optional)");
		final Option module = createOptionWithArg(
				CommandlineProperties.MODULE_OPTION,
				"the module to load on the cluster (optional)");
		final Option killExisting = createOption(
				KILL_POSSIBLY_EXISTING_JOB,
				"k",
				"specify this if you want to kill & clean a possibly existing job with the same name before submitting the job (optional)");
		final Option recheckIntervall = createOptionWithArg(
				TIME_TO_WAIT_BEFORE_RECHECK_STATUS,
				"time to wait inbetween status checks in seconds (default: 600, optional for all|join)");
		final Option stageout = createOption(
				STAGEOUT_OPTION,
				"if you want to stageout the results if the job is finished (optional for modes all|check|join)");
		final Option clean = createOption(
				CLEAN_OPTION,
				"specify this if you want to clean the job after a successful stageout (optional for modes all|check|join)");
		final Option stageOutDirectory = createOptionWithArg(
		...
		final Option generateUniqueJobName = createOption(
				CommandlineProperties.GENERATE_UNIQUE_JOBNAME_OPTION,
				"with this option, grisu client ensures that the name for the job will always be unique.\n"
						+ "the "
						+ CommandlineProperties.JOBNAME
						+ " is still required and will be used as a prefix to this job");
		final Option shibUsername = createOptionWithArg(
				CommandlineProperties.SHIB_USERNAME_OPTION, "u",
				"shibboleth username");
		final Option shibIdp = createOptionWithArg(
				CommandlineProperties.SHIB_IDP_OPTION,
	...
	private final Set<String> failedParameters = null;
	...
	private final String[] outputFileNames = null;
	...
	private void checkCommonOptions() {
		 ...
		 * formatter.printHelp("grisu-client", this.options); System.exit(1); }
		 */
	...
	private void checkStageoutModeParameters() {
		...
		if (line.hasOption(STAGEOUT_OPTION)) {
			...
			final String stageOutDirectoryStr = getConfigOption(STAGEOUTDIRECTORY_OPTION);

			if (!line.hasOption(STAGEOUTDIRECTORY_OPTION)
					&& (stageOutDirectoryStr == null)) {
				myLogger.debug("No stageout directory specified. Using current directory.");
			...
			final File dir = new File(stageOutDirectory);
			if (dir.exists()) {
				if (!dir.canWrite()) {
		...
		clean = "true".equals(getConfigOption(CLEAN_OPTION))
				|| line.hasOption(CLEAN_OPTION);
	...
	private void checkSubmitModeParameters() {
		...
		if (!line.hasOption(CommandlineProperties.INPUTFILEPATH_OPTION)) {
			...
			final String inputFileString = line
					.getOptionValue(CommandlineProperties.INPUTFILEPATH_OPTION);
			if (!StringUtils.isEmpty(inputFileString)) {
				final String[] input = inputFileString.split(",");
				for (final String filepath : input) {

					final File file = new File(filepath);
					if (!file.exists()) {
						System.err.println("Input file " + filepath
								+ " doesn't exist.");
						System.exit(1);
					}
				}
	...
	public boolean debug() {
		final boolean debug = line.hasOption(DEBUG_OPTION);
	...
	public boolean killPossiblyExistingJob() {
		final boolean kill = line.hasOption(KILL_POSSIBLY_EXISTING_JOB);
		return (kill || "true"
				.equals(getConfigOption(KILL_POSSIBLY_EXISTING_JOB)));
	}
	...
	private void parseCLIargs(String[] args) {
		...
		} else if (ALL_MODE_PARAMETER.equals(mode)
				|| FORCE_ALL_MODE_PARAMETER.equals(mode)) {
	...
	public boolean stageOutResults() {
		final boolean stageout = line.hasOption(STAGEOUT_OPTION);
		return (stageout || "true".equals(getConfigOption(STAGEOUT_OPTION)));
	}

	public boolean useLocalProxy() {
		final boolean useLocalProxy = line.hasOption(USE_LOCAL_PROXY_OPTION);
		return (useLocalProxy || "true"
				.equals(getConfigOption(USE_LOCAL_PROXY_OPTION)));
	}

	public boolean verbose() {
		final boolean verbose = line.hasOption(VERBOSE_OPTION);
